<div>
    ...
    <div class="flex flex-row justify-between items-center py-4 bg-orange-400">
        <div>
            <span class="text-lg text-white px-4">Tags <span
                    class="text-sm">({{ $search != '' ? $found : $total }})</span></span>
        </div>
        <div class="px-4">
            <a href="{{ route('codetag.create') }}"
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start px-2 sm:px-4 py-4 gap-4">
        ...
        <div class="relative w-full">
            ...
            <input wire:model.live="search" type="search"
                class="w-full rounded-lg pl-10 placeholder-zinc-400 focus:outline-none focus:ring-0 focus:border-orange-600 border-2 border-zinc-200"
        ...
        <div class="relative w-32">
            ...
            <select wire:model.live="perPage"
                class="w-full rounded-lg text-end focus:outline-none focus:ring-0 focus:border-orange-600 border-2 border-zinc-200 ">
        ...
        <div class="overflow-x-auto">
                    ...
                    <thead>
                        <tr class="text-black text-left text-sm font-normal uppercase">
                            ...
                            <th wire:click="sorting('name')" scope="col"
                                class="p-2 hover:cursor-pointer hover:text-orange-600 {{ $column == 'name' ? 'text-orange-600' : '' }}">
                                name {!! $sortLink !!}</th>
                            <th wire:click="sorting('created_at')" scope="col"
                                class="p-2 hover:cursor-pointer hover:text-orange-600 {{ $column == 'created_at' ? 'text-orange-600' : '' }}">
                                created {!! $sortLink !!}</th>
                            <th wire:click="sorting('updated_at')" scope="col"
                                class="p-2 hover:cursor-pointer hover:text-orange-600 {{ $column == 'updated_at' ? 'text-orange-600' : '' }}">
                                updated {!! $sortLink !!}</th>
                    ...
                    <tbody>
                        ...
                        @foreach ($tags as $tag)
                            <tr
                                ...
                                <td class="p-2"><a href="{{ route('codetag.show', $tag) }}">{{ $tag->name }}</a>
                                </td>
            ...
            @else
                <div class="flex flex-row justify-between items-center bg-black text-white rounded-lg p-4 mx-2 sm:mx-0">
                    <span>No tags found in the system.</span>
