    ...
    @if ($show % 2 != 0)
        <div class="flex flex-row justify-start w-fit pt-4 px-4 sm:px-12">
            <div class="bg-black text-sm text-white p-2 mx-2 rounded-lg relative">
                <span class="text-yellow-400 font-bold">HELP - </span> Add multiple types using the Add button.
    ...
    <div class="mx-auto w-11/12 py-4 px-2">
        ...
        @foreach ($inputs as $key => $value)
            <div class="flex flex-row justify-start items-center gap-2 py-2">
                ...
                <input wire:model="inputs.{{ $key }}.name" type="text" id="inputs.{{ $key }}.name" class="w-full sm:w-1/2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-yellow-500 focus:border-yellow-500 placeholder:text-zinc-400 px-2" placeholder="Enter a name">
                @if ($count > 0)
