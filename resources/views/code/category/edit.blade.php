            ...
            <div class="mx-auto w-11/12 py-4 px-2">
                <form action="{{ route('codecategory.update', $category) }}" method="POST">
                    ...
                    <div class="flex flex-col justify-start items-start w-full sm:w-2/3 gap-4 py-2">
                        ...
                        <input name="name" id="name" type="text" value="{{ $category->name }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-lg w-full sm:w-2/3 p-2 focus:ring-blue-600 focus:border-blue-600">
                    </div>
                    <!-- Errors -->
                    @error('name')
                        <div>
