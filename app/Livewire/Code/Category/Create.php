...
class Create extends Component
{
    ...
    protected $rules = [
        'inputs.*.name' => 'required|min:3|unique:code_categories,name|distinct'
    ];
    ...
    public function save()
    {
        ...
        foreach ($this->inputs as $input) {
            ...
            try {
                CodeCategory::create(['name' => $input['name']]);
            } catch (QueryException $exception) {

                $errorInfo = $exception->errorInfo;
                // Return the response to the client..
                return to_route('codecategory.index')->with('message', 'Error(' . $errorInfo[0] . ') creating the category (' . $input['name'] . ')');
            }
        }
        ...
        return to_route('codecategory.index')->with('message', $this->inputs->count() . $message);
    }
    
    public function render()
    {
        return view('livewire.code.category.create');
    }
}
